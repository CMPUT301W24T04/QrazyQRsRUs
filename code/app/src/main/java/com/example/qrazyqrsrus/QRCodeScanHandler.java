...
public class QRCodeScanHandler{
    ...
    //4 = more than one event with this qr code as their check in
//    private int errorNumber;
     ...
     */
    public QRCodeScanHandler(AppCompatActivity activity, ScanCompleteCallback callback) {
                                ...
                                FirebaseDB.getInstance().findEventWithQR(result.getContents(), 1, new FirebaseDB.MatchingQRCallBack() {
                                    ...
                                    public void onResult(Event matchingEvent) {
                                        ...
                                        if (matchingEvent.getSignUps().contains(user.getDocumentId())){
                                            ...
                                            FirebaseDB.getInstance().userCheckedIntoEvent(user, matchingEvent, isUnique -> {
                                                if (isUnique) {
                                                    callback.onCheckInResult(matchingEvent);
                                                }
                                                else {
    ...
    public String scanImage(ContentResolver cr, Uri uri){
        ...
        //the post was made by the user Hugo Allexis Cardona (https://stackoverflow.com/users/1797127/hugo-allexis-cardona) on the post https://stackoverflow.com/a/55427749
